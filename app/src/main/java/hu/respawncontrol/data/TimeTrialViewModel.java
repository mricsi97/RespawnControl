...
public class TimeTrialViewModel extends AndroidViewModel {
    ...
    public void saveScore() {
        if(!selectedDifficulty.getName().equals("Custom")) {
            repository.insertScore(new Score(solveTimeSum.getValue(), leaderboard.getId()));
        }
    }
    ...
    public void saveResults() {
        ...
        for (int i = 0; i < solveTimes.size(); i++) {
            Result result = new Result(solveTimeDifferenceValues.get(i), testItems.getValue().get(i).getItemId());
